//
//  ViewController.swift
//  DoAlgorithm
//
//  Created by zuo on 2021/12/16.
//

import UIKit

class ViewController: UIViewController {

    override func viewDidLoad() {
        super.viewDidLoad()
        // Do any additional setup after loading the view.
//        var result = TwoPointer().longestPalindrome("1")
//        result = TwoPointer().longestPalindrome("""
//        
//        """)
//        result = TwoPointer().longestPalindrome("12345")
//        result = TwoPointer().longestPalindrome("123328")
//        result = TwoPointer().longestPalindrome("1233289")
//
//        var has = Stack().isValid("()")
//        has = Stack().isValid("(")

//        print(result)
    }
    
    
    @IBAction func test(_ sender: UIButton) {
        
//        print(DFS().permute([1,2]))
        print(Tree().buildTree([1,2], [1,2]))
    }
    


}

